...
<%-- 공통 페이지네이션, 호출 전 pg 와 pgBase 필수, pgQuery 는 선택 --%>
<c:set var="pgQ" value="${empty pgQuery ? '' : pgQuery}" />
<c:if test="${pg.totalPages > 1}">
<nav class="pk-paging" aria-label="페이지 목록">
    <c:choose>
        <c:when test="${pg.hasPrev}">
            <a href="${pgBase}?page=${pg.prevPage}${pgQ}" class="pk-page pk-page--arrow" aria-label="이전 페이지">‹</a>
        </c:when>
        <c:otherwise>
    ...
    <c:forEach begin="${pg.blockStart}" end="${pg.blockEnd}" var="i">
        <c:choose>
            ...
            <c:otherwise>
                <a href="${pgBase}?page=${i}${pgQ}" class="pk-page">${i}</a>
            </c:otherwise>
        </c:choose>
    </c:forEach>
    ...
    <c:choose>
        <c:when test="${pg.hasNext}">
            <a href="${pgBase}?page=${pg.nextPage}${pgQ}" class="pk-page pk-page--arrow" aria-label="다음 페이지">›</a>
        </c:when>
        <c:otherwise>
            <span class="pk-page pk-page--arrow is-disabled" aria-hidden="true">›</span>
        </c:otherwise>
